...
function MyDungeonsBook:AllDebuffsOnPartyMemberFrame_Create(parentFrame, challengeId)
    local allDebuffsOnPartyMemberFrame, descriptionLabel = self:TabContentWrapperWidget_Create(parentFrame);
    descriptionLabel:SetText(L["List of all debuffs gotten by party members."]);
    ...
    local table = self:TableWidget_Create(columns, 10, 40, nil, allDebuffsOnPartyMemberFrame, "all-debuffs-on-party-members");
    table:SetData(data);
