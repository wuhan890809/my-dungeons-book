...
function MyDungeonsBook:HealByPartyMemberFrame_Create(parentFrame, challengeId, unitId)
    ...
    local data = self:HealByPartyMemberFrame_GetDataForTable(challengeId, "PARTY-MEMBERS-HEAL", unitId);
    local columns = self:HealByPartyMemberFrame_GetColumnsForTable(challengeId);
    local table = self:TableWidget_Create(columns, 11, 40, nil, healByPartyMemberFrame, "heal-by-" .. unitId);
    table:SetData(data);
    table:RegisterEvents({
        OnEnter = function (rowFrame, cellFrame, data, cols, row, realrow, column)
            if (realrow) then
                ...
                if (column > 3) then
                    self:HealByPartyMemberFrame_HealHover(rowFrame, cellFrame, data, cols, row, realrow, column);
                end
            end
        end,
...
function MyDungeonsBook:HealByPartyMemberFrame_HealHover(_, cellFrame, data, cols, _, realrow, column)
...
function MyDungeonsBook:HealByPartyMemberFrame_GetColumnsForTable(challengeId)
...
function MyDungeonsBook:HealByPartyMemberFrame_GetDataForTable(challengeId, key, unitId)
    local tableData = {};
