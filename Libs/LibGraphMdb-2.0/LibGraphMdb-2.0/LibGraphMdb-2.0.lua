...
local major = "LibGraphMdb-2.0"
local minor = 90000 + tonumber(("$Revision: 58 $"):match("(%d+)"))


--Search for just Addon\\ at the front since the interface part often gets trimmed
--Do this before anything else, so if it errors, any existing loaded copy of LibGraphMdb-2.0
...
local TIME_FORMATTER = "TIME_FORMATTER";
...
local function SetupGraphRealtimeFunctions(graph, upgrade)
	...
	graph.FormatXAxis = GraphFunctions.FormatXAxis
	graph.SetFormatXAxisAsTime = GraphFunctions.SetFormatXAxisAsTime
...
-- SetFormatXAxisAsTime - X Axis Legend should be output as time ("hh:mm:ss" or "mm:ss")
function GraphFunctions:SetFormatXAxisAsTime()
	self.xAxisFormat = TIME_FORMATTER;
end

-- FormatXAxis - format value for X Axis
function GraphFunctions:FormatXAxis(xAxisValue)
	if (self.xAxisFormat == TIME_FORMATTER) then
		local timeFormat = "%M:%S";
		local time = xAxisValue;
		if (time > 3600) then
			return date("%S", math.floor(time / 3600 + 0.5)) .. ":" .. date(timeFormat, time % 3600);
		end
...
--To test the library do /script LibStub("LibGraphMdb-2.0"):TestGraph2Lib()
